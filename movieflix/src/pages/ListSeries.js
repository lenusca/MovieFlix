...
class ListSeries extends Component{
    ...
    componentDidMount(){
        fetch("http://localhost:8888/Series/").then(response=> response.json()).then(data => this.setState({series: data, isLoading:false}))
    }
    render (){
        return(
            <div>
                            ...
                            <div class="grid-container" style={{}}>
                                {!this.state.isLoading?(
                                    this.state.series.map(
                                        user => {
                                            return (
                                                <div class ="grid-item" style={{position:"relative",right:"150px", marginRight:"-200px"}}><a href={"DetailSerie?id="+user.imdbID}><img alt=" " src={user.Poster} style={{width:"350 px", height:"350px"}}/></a><p style={{color:"white"}}>{user.Title}<p><small style={{fontSize: "20px", fontWeight: "bolder",color:"grey"}}>{user.Year}</small></p></p></div>        
                                            );
                                        })
                                ):(
                                    <h3>Loading...</h3>
